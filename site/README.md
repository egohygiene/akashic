...
`atlas.html` is a separate place-aware explorer. Its hierarchy lives in `atlas/locations.json`, explicit main-catalog resource/place associations live in `atlas/applicability.json`, resources canonical to one place remain Markdown under `atlas/places/`, and visual palettes live in `site/data/atlas-themes.json`. The build validates and joins those sources into `dist/data/atlas.json`; do not edit that generated file. Compact checked-in TopoJSON keeps the atlas private, static, and independent of paid map services. Use `node scripts/fetch-atlas-geometry.mjs` only when intentionally refreshing the pinned upstream geometry.
